...
export const EquipmentController = {
  getAll: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const [items, accessibleCount] = await Promise.all([
        EquipmentService.getAll(req.user!.id),
        EquipmentService.countAccessible(req.user!.id),
      ])
      res.json({ data: items, accessible_exercises: accessibleCount })
    } catch (err) { next(err) }
  },

  create: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const item = await EquipmentService.create(req.user!.id, req.body)
